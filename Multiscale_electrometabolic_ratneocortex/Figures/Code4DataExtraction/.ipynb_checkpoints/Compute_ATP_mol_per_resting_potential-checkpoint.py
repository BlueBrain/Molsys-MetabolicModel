...
def Find_mol_per_AP(row_idx):
    ...
    if no_ap_window is not None:
        ...
        # # Debug output to check the shape and type of the result
        # if isinstance(molecules_consumed, (list, np.ndarray)):
        #     print(f"Row {row_idx}: Molecules consumed shape: {np.shape(molecules_consumed)}")
        # else:
        #     print(f"Row {row_idx}: Molecules consumed: {molecules_consumed}")
...
print(len(nodes_ids))
result_path ='/gpfs/bbp.cscs.ch/data/scratch/proj137/farina/Met_Aug/v03/sim14/reporting_3000_node_sets_v03_single_node_removed/'
